...
DXGI_FORMAT CDeviceResources::NoSRGB(DXGI_FORMAT fmt) noexcept {
    switch (fmt) {
...
void CDeviceResources::InitializeDXGIAdapter(IDXGIFactory4* factory) noexcept(false) {
...
void CDeviceResources::InitializeAdapter(IDXGIAdapter1** ppAdapter, DXGI_GPU_PREFERENCE preference) noexcept(false) {
...
void CDeviceResources::MoveToNextFrame() noexcept(false) {
...
HRESULT __stdcall CDeviceResources::CreateDeviceResources(IDXGIAdapter1* adapter) noexcept {
    try {
        ...
        if (adapter == nullptr) {
            if (adapter = m_adapter.get(); adapter == nullptr)
