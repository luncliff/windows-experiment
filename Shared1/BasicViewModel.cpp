...
IAsyncAction BasicViewModel::CreateLogFolderAsync() {
    try {
        ...
        // Use logging channel interface instead of spdlog
        auto logger = winrt::App1::make_stream_logging_channel(std::wcout, L"BasicViewModel");
        logger.LogMessage(L"Log folder created: " + GetLogFolderPath());
    } catch (const winrt::hresult_error& ex) {
        auto logger = winrt::App1::make_stream_logging_channel(std::wcout, L"BasicViewModel");
        logger.LogMessage(L"Failed to create logs folder: " + ex.message());
    }
}
